...
- `v4.5.57 (2026-03-30)` 农场土地工作台重构、单地块操作与高风险窗口扩展版：农场页新增智能筛选/排序、单块与批量土地操作、阶段进度展示；QQ 高风险窗口支持超长时长和“不限制”模式，并统一当前版本口径到 `v4.5.57`。
- `v4.5.56 (2026-03-30)` 设置总控工作台收口与发布链路续发版：把管理员总控工作台从概览页迁到设置页“账号与安全”，统一当前版本口径到 `v4.5.56`，作为本地最新版本继续发布 GitHub / Docker / 更新公告。
...
### 开发补记 - v4.5.57 农场土地工作台重构、单地块操作与高风险窗口扩展版 (2026-03-30)

#### ✅ 本轮农场工作台升级
- ✅ **农场页已升级为可直接处理的土地工作台**: `FarmPanel.vue` 新增智能排序、问题快捷筛选、紧凑/舒适视图切换、批量选择与批量操作，管理员可以更快聚焦可收、待照料、快成熟与空闲/枯萎土地。
- ✅ **土地卡片已补齐阶段进度、时间线与快捷操作**: `LandCard.vue` 现统一展示整株成长进度、阶段进度、阶段剩余时间、土地类型、状态徽标和详情抽屉；好友页展开土地详情时也复用了这套视觉，但保持只读。
- ✅ **后端单地块操作链路已接通**: `farm.js / worker.js / data-provider.js / account-control-routes.js` 已新增单地块收获、浇水、除草、除虫能力，前端批量操作会在此基础上稳定串行执行。

#### ✅ 本轮高风险窗口与设置收口
- ✅ **QQ 高风险窗口支持更长时长和“不限制”模式**: 存储层、配置校验与设置页已统一支持 `5 ~ 43200` 分钟，以及 `0 = 不限制` 的模式；选择不限制后，后端不会再自动关闭这 4 项高风险开关。
- ✅ **设置页提示与确认文案已按真实行为重写**: `Settings.vue` 新增时长预设、无限时切换、摘要说明和更明确的保存确认文案，避免把“长时授权”和“手动收口模式”混为一谈。
- ✅ **系统日志已能正确显示无限时授权**: `SystemLogs.vue` 现会把 `durationMinutes=0` 显示为“不限制”，不再误导成 0 分钟。

#### ✅ 本轮种子/背包/资源兜底
- ✅ **缺失 `Plant.json` 的稀有种子会继续保留正确尺寸**: `gameConfig.js` 新增种子尺寸兜底逻辑，像爱心果这类缺失原始植物配置的作物，仍能稳定保留 `2x2` 占地信息。
- ✅ **背包可种种子清单不再误伤已拥有的高等级种子**: `warehouse.js` 已按真实持有情况暴露 `usableCount / plantSize`，避免背包里明明有稀有种子，却在种植清单里被等级门槛误判不可种。
- ✅ **未知物品 ID 现在会留下更容易排查的调试日志**: 遇到未收录的背包物品时，会额外记录邻近条目、批次内未知 ID 和占位图回退信息，方便后续补齐配置与图标素材。

#### ✅ 本轮发布口径
- ✅ **当前默认版本已统一切到 `v4.5.57`**: `core/package.json`、`web/package.json`、README、部署模板、Docker 发布脚本、GitHub Actions 默认版本与帮助中心 Release Notes 已同步抬升。
- ✅ **本轮会继续承接 GitHub / Docker / 双机部署主线**: 这次版本将作为新的正式对外版本，用于 GitHub Release、Docker Hub 镜像推送、更新公告追加，以及两台服务器的最新程序部署与升级验证。

### 开发补记 - v4.5.56 设置总控工作台收口与发布链路续发版 (2026-03-30)

#### ✅ 本轮界面收口
- ✅ **管理员总控工作台已从 `Dashboard.vue` 收口到 `Settings.vue`**: 概览页移除了用户/卡密/公告/更新总控面板，避免运行态首页和管理面板混在一起。
- ✅ **设置页“账号与安全”新增管理员总控工作台**: 集中展示用户数、卡密库存、体验卡状态、公告状态、帮助反馈、系统更新摘要，并保留快捷跳转入口与最近操作时间线。
- ✅ **卡密总控保存后会同步刷新管理员总控摘要**: 调整总控开关后，设置页里的管理员摘要卡片会及时反映最新状态。

#### ✅ 本轮发布收口
- ✅ **当前默认版本已统一切到 `v4.5.56`**: `core/package.json`、`web/package.json`、README、部署模板、Docker 发布脚本、GitHub Actions 默认版本与帮助中心 Release Notes 已同步抬升。
- ✅ **这次不是推翻 `v4.5.55`，而是在其发布准备基础上继续迭代**: 新增的设置总控工作台改动已被并入“本地最新版本”，避免未提交的真实界面调整遗漏在正式发布之外。
...
#### ✅ 本轮发布收口
- ✅ **推送渠道成功/失败判定已统一归一化**: `sendPushooMessage()` 现在会识别 `ok / success / errcode / errno / code / status / result` 等返回字段，不再把“`errmsg: ok` 但实际上失败”的响应误判为成功。
- ✅ **Webhook HTTP 失败信息更可读**: 自定义 Webhook 返回 `401 / 403 / 500` 等异常状态时，会把 `http_<status>` 与响应体摘要一起带回，后台和测试里更容易定位第三方拒绝原因。
- ✅ **汇报发送器抛异常时也会落失败日志**: `report-service` 不再因为发送层直接抛错就跳过投递记录，测试汇报与后续失败分析路径更完整。
- ✅ **本地最新工作区的发布默认版本已抬升到 `v4.5.53`**: 包版本、README、部署模板、离线包脚本、自动更新脚本、GitHub Actions 默认值、帮助中心 Release Notes 与运行态更新公告已经同步到 `v4.5.53`。
...
#### ✅ 本轮发布收口
- ✅ **最近多轮功能更新已统一收口到新的发布版本**: `v4.5.47` 到 `v4.5.51` 的 QQ 好友导入 / 巡检台增强 / 卡密发放总控等最近更新，现在都能通过统一版本口径对外说明、部署与回滚。
- ✅ **部署模板和工作流默认值已全部抬升到 `v4.5.52`**: `core/package.json`、`web/package.json`、README、`deploy/docker-compose.yml`、`.env.example`、离线包脚本和 GitHub Actions 默认版本已经对齐。
- ✅ **多实例升级口径继续补强**: 本轮特别覆盖 `2400 / 2500 / 2600` 多实例场景，避免主程序和子程序在部署模板、系统更新提示和默认镜像标签上继续错位。
- ✅ **共享网络复用不再卡在 Compose 标签冲突**: 安装与更新脚本检测到同名共享网络已存在时，会自动切到 `external` 复用模式，真实多栈 / 自测场景不会再因为旧网络标签不匹配而直接失败。

#### 📌 本轮发布说明
- 📌 **这轮更偏向“版本与发布链路收口”**: 不是单点功能新增，而是把最近多轮真实改动整理成可发布、可部署、可核验的一版。
- 📌 **这轮也是双机升级前的发布整理版**: 目的是把 GitHub / Docker / 部署模板 / 更新公告 / 服务器安装看到的版本统一到同一个基线。

### 开发补记 - v4.5.38 远程更新 smoke 无 Node 兜底与默认地址修复 (2026-03-25)

#### ✅ 本轮发布收口
- ✅ **宿主机无 Node 的 smoke 已真正可用**: `smoke-system-update-center.sh` 会自动在宿主机 `node` 和主程序容器内置 `node` 之间切换，联动检查不再依赖服务器额外安装 Node.js。
- ✅ **smoke 的登录续用链路已修平**: 首个登录请求就会正确写入 cookie jar，后续 `/api/auth/validate`、`/api/admin/system-update/*` 不会再出现“登录成功但全是 401”的假失败。
- ✅ **默认地址与文档口径已统一**: smoke 脚本会结合 `--deploy-dir` 自动读取 `.env` 中的 `WEB_PORT`，设置页、帮助中心、SOP 和部署脚本输出也已经去掉旧的 `9527` 硬编码。
- ✅ **版本口径已抬升到 `v4.5.38`**: `core/package.json`、`web/package.json`、README、部署模板、构建脚本和默认镜像标签已经全部统一到 `v4.5.38`。

#### 🩹 review 后继续修掉的问题
- ✅ **不是只有“无 Node”一个洞，cookie 首登也一起修了**: 真机复测时发现 `/api/login` 成功后 admin 接口仍返回 `401`，最终定位到脚本只在 cookie 文件“已存在”时才附加 `-c/-b`，首个登录请求根本不会保存 cookie。
- ✅ **旧 smoke 默认地址会把管理员带去错误端口**: 服务器实际默认对外端口是 `3080`，但脚本、帮助中心和设置页复制命令还残留了 `9527`，这次已经改成按 `WEB_PORT` 自动解析。
- ✅ **真实服务器真机回归已通过**: 在 `aa.smdk000.com` 上，宿主机无 `node` 的情况下，更新中心 smoke 已能通过登录、认证验证、更新概览、检查更新、公告 dry-run、预检和 `verify-stack`；当前仅保留“暂无更新任务”这一条提醒。

#### 📌 本轮发布说明
- 📌 **这轮是对远程更新验收链路的最后一层收口**: 不新增页面能力，重点是把“管理员照文档执行 smoke 就能跑通”这件事做实。
- 📌 **这轮修复直接来自真实服务器复测**: 先发现宿主机无 `node`，再发现旧端口示例和 cookie jar 首登问题，所以最终是脚本兜底、默认地址和文档入口一起修。
